...
const GLfloat t_CubeVCNT[]=
{
 ...
 int i=0,j=0;
        ...
        { 
            for (int k =0 ; k <3 ; k++)
             {
                cubeVCNT[j].vertex[k] = t_CubeVCNT[i];
                fprintf(gpFile,"cubeVCNT[%d].vertex[%d] : %f \t",j,k,cubeVCNT[j].vertex[k]);
                i++;
             }
            for(int k = 0; k < 3; k++)
            {
                cubeVCNT[j].color[k] = t_CubeVCNT[i];
                fprintf(gpFile,"cubeVCNT[%d].color[%d] : %f \t",j,k,cubeVCNT[j].color[k]);
                i++;
            }
            for(int k = 0; k< 3; k++)
            {
                cubeVCNT[j].normal[k] = t_CubeVCNT[i];
                fprintf(gpFile,"cubeVCNT[%d].normal[%d] : %f \t",j,k,cubeVCNT[j].normal[k]);
                i++;
            }
            for(int k =0 ;k < 2; k++)
            {
                cubeVCNT[j].texture[k] = t_CubeVCNT[i];
                fprintf(gpFile,"cubeVCNT[%d].texture[%d] : %f \n",j,k,cubeVCNT[j].texture[k]);
                i++;
            }
