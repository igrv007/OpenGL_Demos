	...
	switch(iMsg)
	{
        ...
        case VK_UP:
          ...
          if(gNumberOfLineSegments >= 50)
            gNumberOfLineSegments =50; //reset no.of line segments to 50
        break;
        case VK_DOWN:
        gNumberOfLineSegments--;
        if(gNumberOfLineSegments <=0 )
          gNumberOfLineSegments =1; // reset no.of line segments to 0
        break;
                default:
                break;
			}
...
void initialize(void)
{
    ...
    const GLchar *vertexShaderSourceCode=
          ...
          //get the shader compilation status of vertexShaderObject
           ...
           glGetShaderiv(gTessellationControlShaderObject,GL_COMPILE_STATUS,&iShaderCompilatedStatus);
           if(iShaderCompilatedStatus ==GL_FALSE)
           {
           ...
           //provide source code to shader
         const GLchar *tessllationEvaluationShaderSourceCode = 
           "#version 430 core" \
           "\n" \
           "layout(isolines)in;" \
           ...
           glShaderSource(gTessellationEvaluationShaderObject,1,(const GLchar **)&tessllationEvaluationShaderSourceCode,NULL);
           ...
           iShaderCompilatedStatus = 0;
           szInfoLog = NULL;
           glGetShaderiv(gTessellationEvaluationShaderObject,GL_COMPILE_STATUS,&iShaderCompilatedStatus);
           if(iShaderCompilatedStatus == GL_FALSE)
           {
    ...
    //attach tessellation evalution shader to shader program
    ...
    //get numvber of strips uniform location
...
void display(void)
{
	...
	//code
     ...
     translationMatrix = modelViewMatrix * vmath::translate(0.5f,0.5f,-2.0f);
...
glDrawArrays(GL_PATCHES,0,4);  //3(each with its x,y,z) vertices in triangle vertices array
