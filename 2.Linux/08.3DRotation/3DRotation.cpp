...
//get binded with our enum variable which is linked to "vPosition" variable from vertex shader code
        ...
        glGenBuffers(1,&gVbo_pyramid_color);//i want to place the data in GPU so ask GPU to give me the buffer
                             //to store my vertex,normal,color,texture data . In return GPU will return 
                             //a key in "gVbo".
        {
          check_gl_error();

          glBindBuffer(GL_ARRAY_BUFFER,gVbo_pyramid_color);//get binded with that key i.e gVbo
...
//S6.address of interleaved data buffer
  ...
  glGenBuffers(1,&gVbo_cube_color);//i want to place the data in GPU so ask GPU to give me the buffer
                             //to store my vertex,normal,color,texture data . In return GPU will return 
                             //a key in "gVbo".
  check_gl_error();
  {
    glBindBuffer(GL_ARRAY_BUFFER,gVbo_cube_color);//get binded with that key i.e gVbo
    check_gl_error();
    {
      //now following operations will affect this gVbo
   //Following function will send our data from CPU to GPU
...
void display(void)
{
 ...
 rotationalMatrix = modelViewMatrix * vmath::rotate(angle_pyr,0.0f,1.0f,0.0f);
...
glDrawArrays(GL_TRIANGLES,0,12); //3 vertices
check_gl_error();
//unbind vao
glBindVertexArray(0);
check_gl_error();
...
rotationalMatrix = modelViewMatrix * vmath::rotate(angle_cub,angle_cub,angle_cub);
//modelViewMatrix=modelViewMatrix * rotationalMatrix;

scaleMatrix=rotationalMatrix* vmath::scale(0.75f,0.75f,0.75f);
