...
{ 
  ...
  //getnew framebuffer config that meets our attrib requirements
    ...
    int bestFramebufferconfig=-1,worstFramebufferConfig=-1,bestNumberOfSamples=-1,worstNumberOfSamples=999;
     ...
     XStoreName(gpDisplay,gWindow,"OpenGL Window Programmable Pipeline -Black and White Triangle");
...
if(iShaderCompilatedStatus == GL_FALSE)
{   
  //get th error in the parameterized return value
...
glAttachShader(gShaderProgramObject,gFragmentShaderObject);
    ...
    //get the linking error stauts in parameterized variable
  ...
  glGenBuffers(1,
               &gVbo);//i want to place the data in GPU so ask GPU to give me the buffer
...
//unibind the GL_ARRAY_BUFFER by passing "0" as second parameter
...
glGenVertexArrays(1,              //No. of vao
                 &gVao_Square);//i want to run following  instructions letsay "x" no of instructions 
                             //so give me the key to run those instructions repeatedly.
