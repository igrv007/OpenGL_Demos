...
{ 
  ...
  //getnew framebuffer config that meets our attrib requirements
    ...
    int bestFramebufferconfig=-1,worstFramebufferConfig=-1,bestNumberOfSamples=-1,worstNumberOfSamples=999;
     ...
     XStoreName(gpDisplay,gWindow,"OpenGL Window Programmable Pipeline -Color Triangle");
...
//get the shader compilation status of vertex shader object
   ...
   //provide a source code to sahder
const GLchar *fragmentShaderSourceCode=
        ...
        //give above fragment shader source code to fFragmentShaderObject
...
if(iShaderCompilatedStatus == GL_FALSE)
{
  ...
  if(iInfoLogLength > 0)
  {
    ...
    if(szInfoLog !=NULL)
    {
      ...
      fprintf(gpFile, "Fragment compilation Log:%s\n",szInfoLog );
      free(szInfoLog);
      uninitialize();
      exit(0);
    }
      ...
      //get the error 
            ...
            //S3: pointer variable to hold the error string
              glGetProgramInfoLog(gShaderProgramObject,
                             ...
                             &written,
                             szInfoLog);
              fprintf(gpFile, "Shade program Link Log:%s\n",szInfoLog );
              free(szInfoLog);
              uninitialize();
              exit(0);
    }
...
glGenVertexArrays(1,
                &gVao_triangle);//i want to run following  instructions    letsay "x" no of instructions 
...
glGenBuffers(1,
          &gVbo_triangle_position);//i want to place the data in GPU so ask GPU to give me the buffer
                             //to store my vertex,normal,color,texture data . In return GPU will return 
                             //a key in "gVbo".
glBindBuffer(GL_ARRAY_BUFFER,
           gVbo_triangle_position);//get binded with that key i.e gVbo
...
glGenBuffers(1,
          &gVbo_triangle_color);//i want to place the data in GPU so ask GPU to give me the buffer
                             //to store my vertex,normal,color,texture data . In return GPU will return 
                             //a key in "gVbo".
...
void resize(int width, int height)
{
  ...
  //glPerspective
  gPerspectiveProjectionMatrix=perspective(60.0f,(GLfloat)width/(GLfloat)height,0.1f,150.0f);
