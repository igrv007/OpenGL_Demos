...
{ 
  ...
  //getnew framebuffer config that meets our attrib requirements
    ...
    int bestFramebufferconfig=-1,worstFramebufferConfig=-1,bestNumberOfSamples=-1,worstNumberOfSamples=999;
     ...
     XStoreName(gpDisplay,gWindow,"OpenGL Window Programmable Pipeline -Kundali and Stone Texture");
...
//get MVP uniform location
    ...
    gLdUniform = glGetUniformLocation(gShaderProgramObject,
    	"u_Ld");
    gKdUniform = glGetUniformLocation(gShaderProgramObject,
    	"u_Kd");

    gLightPositionUniform= glGetUniformLocation(gShaderProgramObject,
    	                                          "u_light_position");
...
for (int  i=0;i<72;i++)
 {
 	if(cubePosition[i]<0.0f)
 		cubePosition[i]=cubePosition[i] + 0.25f;
 	else if(cubePosition[i]>0.0f)
 		cubePosition[i]=cubePosition[i] - 0.25f;
 	else
 		cubePosition[i] = cubePosition[i];
 }
...
//S6.address of interleaved data buffer
  ...
  check_gl_error();
  {
    glBindBuffer(GL_ARRAY_BUFFER,gVbo_cube_normal);//get binded with that key i.e gVbo
    check_gl_error();
    {
      //now following operations will affect this gVbo
   //Following function will send our data from CPU to GPU
...
//OpenGL drawing 
   ...
   rotationalMatrix = rotate(angle_cub,angle_cub,angle_cub);

   modelViewMatrix = modelMatrix * rotationalMatrix;  
